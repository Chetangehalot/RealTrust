...
function SubscriptionViewer() {
  ...
  const exportToCSV = () => {
    ...
    // Create CSV headers
    const headers = ['Email Address', 'Subscribed Date', 'Subscribed Time'];
    ...
    <div>
      ...
      {subscriptions.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-xl shadow-md border border-gray-200">
          ...
          <p className="text-gray-500 text-lg font-medium">No subscriptions yet</p>
          <p className="text-gray-400 text-sm mt-2">Subscriptions will appear here when users subscribe to your newsletter</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
          ...
          <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
            <p className="text-sm text-gray-600">
              Total Subscriptions: <span className="font-semibold text-primary-blue">{subscriptions.length}</span>
            </p>
          </div>
        </div>
      )}
